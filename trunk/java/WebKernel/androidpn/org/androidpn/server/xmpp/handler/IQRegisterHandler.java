...
import org.androidpn.service.UserNotFoundException;
...
public class IQRegisterHandler extends IQHandler {
    ...
    public IQRegisterHandler() {
    	apnUserService = ServiceLocator.getInstance().getApnUserService();
        ...
        if (IQ.Type.get.equals(packet.getType())) {
            ...
            try {
                ...
                if (query.element("remove") != null) {
                    ...
                    ApnUser user;
                    if (session.getStatus() == Session.STATUS_AUTHENTICATED) {
                        user = apnUserService.getUserById(session.getUsername());
                        user.setMybatisMapperId(Variable.UPDATE);
                        System.out.println("老用户:"+username+" 上线");
                    } else {
                        user = new ApnUser();
                        user.setMybatisMapperId(Variable.INSERT);
                        System.out.println("新用户:"+username+" 加入");
                    }
