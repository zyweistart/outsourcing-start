...
public class ScaleImage {
	 ...
	 * @param saveToFileStr 	 生成缩略图保存的地址
	...
	public void saveImageAsJpg(String fromFileStr, String saveToFileStr, int formatWideth, int formatHeight) throws Exception {
		...
		srcImage = this.imageZoomOut(srcImage, changeToWideth, changeToHeight);
	...
	public BufferedImage imageZoomOut(BufferedImage srcBufferImage, int w, int h) {
		...
		if (this.DetermineResultSize(w, h) == 1) { return srcBufferImage; }
		this.CalContrib();
		final BufferedImage pbOut = this.HorizontalFiltering(srcBufferImage, w);
		final BufferedImage pbFinalOut = this.VerticalFiltering(pbOut, h);
		return pbFinalOut;
	}
	...
	private int DetermineResultSize(int w, int h) {
		...
		scaleH = (double) w / (double) this.width;
		scaleV = (double) h / (double) this.height;
	...
	private void CalContrib() {
		...
		final int center = this.nHalfDots;
		this.contrib[center] = 1.0;

		double weight = 0.0;
		int i = 0;
		for (i = 1; i <= center; i++) {
			this.contrib[center + i] = this.Lanczos(i, this.width, this.scaleWidth, this.support);
	...
	private int HorizontalFilter(BufferedImage bufImg, int startX, int stopX, int start, int stop, int y, double[] pContrib) {
		...
		for (i = startX, j = start; i <= stopX; i++, j++) {
			...
			valueRed += this.GetRedValue(valueRGB) * pContrib[j];
			valueGreen += this.GetGreenValue(valueRGB) * pContrib[j];
			valueBlue += this.GetBlueValue(valueRGB) * pContrib[j];
		}

		valueRGB = this.ComRGB(this.Clip((int) valueRed), this.Clip((int) valueGreen), this.Clip((int) valueBlue));
		return valueRGB;
	...
	private BufferedImage HorizontalFiltering(BufferedImage bufImage, int iOutW) {
		...
		for (int x = 0; x < iOutW; x++) {
			...
			if ((start > 0) || (stop < this.nDots - 1)) {
				this.CalTempContrib(start, stop);
				for (y = 0; y < dwInH; y++) {
					value = this.HorizontalFilter(bufImage, startX, stopX, start, stop, y, this.tmpContrib);
					pbOut.setRGB(x, y, value);
				}
			} else {
				for (y = 0; y < dwInH; y++) {
					value = this.HorizontalFilter(bufImage, startX, stopX, start, stop, y, this.normContrib);
					pbOut.setRGB(x, y, value);
				}
			}
		}
	...
	private BufferedImage VerticalFiltering(BufferedImage pbImage, int iOutH) {
		...
		for (int y = 0; y < iOutH; y++) {
			...
			if ((start > 0) || (stop < this.nDots - 1)) {
				this.CalTempContrib(start, stop);
				for (int x = 0; x < iW; x++) {
					value = this.VerticalFilter(pbImage, startY, stopY, start, stop, x, this.tmpContrib);
					pbOut.setRGB(x, y, value);
				}
			} else {
				for (int x = 0; x < iW; x++) {
					value = this.VerticalFilter(pbImage, startY, stopY, start, stop, x, this.normContrib);
					pbOut.setRGB(x, y, value);
				}
			}
